...
class NIIPatchDataset(Dataset):
    """Dataset for 3D patches loaded from .nii.gz or .npy files.
    ...
    def __init__(
        ...
        """
        Args:
            patches_dir: Directory containing patch files (patches/)
        ...
        for patch_info in self.patches_info:
            # Use filename directly from patches_info.json
            # The json should contain correct filenames (.nii.gz or .npy depending on preprocess)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        try:
            ...
            filename_str = str(patch_file)
            
            if filename_str.endswith('.npy'):
                # Load NPY file - format is (H, W, D, C) = (256, 256, 32, 3)
                patch = np.load(filename_str)
                # Convert to PyTorch format: (C, D, H, W)
                if patch.ndim == 4:  # (H, W, D, C)
                    patch = np.transpose(patch, (3, 2, 0, 1))  # → (C, D, H, W)
                elif patch.ndim == 3:  # (H, W, D)
                    patch = np.transpose(patch, (2, 0, 1))  # → (D, H, W)
                    patch = np.expand_dims(patch, axis=0)  # → (1, D, H, W)
            else:
                # Load NIfTI file (.nii or .nii.gz)
                img = nib.load(filename_str)
