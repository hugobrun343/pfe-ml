...
def copy_config(config_path: Path, data_dir: Path) -> None:
    """
    Copy the training config YAML to run's data directory.
    
    Args:
        config_path: Path to the config YAML file
        data_dir: Directory where to copy the file
    """
...
def get_runs_directory(project_root: Path) -> Path:
    ...
    Args:
        project_root: Path to gpu-trainer project root
        
    Returns:
        Path to _runs directory
    """
    # _runs is at work-hugo root, which is 2 levels up from gpu-trainer
