"""
SE-ResNet3D-101 for 3D Medical Image Classification

Architecture: ResNet-101 with Squeeze-and-Excitation blocks adapted for 3D inputs
...
Layer configuration: [3, 4, 23, 3] bottleneck blocks with SE attention
- Layer 1: 3 SE-blocks, 256 channels
- Layer 2: 4 SE-blocks, 512 channels  
- Layer 3: 23 SE-blocks, 1024 channels (main difference from SE-ResNet-50)
- Layer 4: 3 SE-blocks, 2048 channels

Total parameters: ~90M (ResNet-101 + SE overhead)
...
class SEResNet3D101(nn.Module):
    """SE-ResNet3D-101 for binary classification with unique weights per block"""
    ...
    def __init__(self, in_channels: int = 3, num_classes: int = 1):
        ...
        for i in range(2):
            self.layer1_blocks.append(nn.ModuleDict({
                'conv1': nn.Conv3d(256, 64, kernel_size=1, bias=False),
                'bn1': nn.BatchNorm3d(64),
                'conv2': nn.Conv3d(64, 64, kernel_size=3, stride=1, padding=1, bias=False),
                'bn2': nn.BatchNorm3d(64),
                'conv3': nn.Conv3d(64, 256, kernel_size=1, bias=False),
        ...
        # Block 1 (with stride=2): 256 -> 512 channels
        # conv1 (1×1×1): 256 → 128
        ...
        for i in range(3):
            self.layer2_blocks.append(nn.ModuleDict({
                'conv1': nn.Conv3d(512, 128, kernel_size=1, bias=False),
                'bn1': nn.BatchNorm3d(128),
                'conv2': nn.Conv3d(128, 128, kernel_size=3, stride=1, padding=1, bias=False),
                'bn2': nn.BatchNorm3d(128),
                'conv3': nn.Conv3d(128, 512, kernel_size=1, bias=False),
                'bn3': nn.BatchNorm3d(512),
                'se': SEBlock3D(512),
            }))

        # ============================================================================
        # LAYER 3: (B, 512, 4, 32, 32) -> (B, 1024, 2, 16, 16) - 23 SE-blocks
        # ============================================================================
        
        # Block 1 (with stride=2): 512 -> 1024 channels
        # conv1 (1×1×1): 512 → 256
        ...
        # Blocks 2-23 (identity) - UNIQUE WEIGHTS (22 blocks)
        self.layer3_blocks = nn.ModuleList()
        for i in range(22):
            self.layer3_blocks.append(nn.ModuleDict({
                'conv1': nn.Conv3d(1024, 256, kernel_size=1, bias=False),
                'bn1': nn.BatchNorm3d(256),
                'conv2': nn.Conv3d(256, 256, kernel_size=3, stride=1, padding=1, bias=False),
                'bn2': nn.BatchNorm3d(256),
                'conv3': nn.Conv3d(256, 1024, kernel_size=1, bias=False),
        ...
        # Block 1 (with stride=2): 1024 -> 2048 channels
        ...
        for i in range(2):
            self.layer4_blocks.append(nn.ModuleDict({
                'conv1': nn.Conv3d(2048, 512, kernel_size=1, bias=False),
                'bn1': nn.BatchNorm3d(512),
                'conv2': nn.Conv3d(512, 512, kernel_size=3, stride=1, padding=1, bias=False),
                'bn2': nn.BatchNorm3d(512),
                'conv3': nn.Conv3d(512, 2048, kernel_size=1, bias=False),
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass: (B, 3, 32, 256, 256) -> (B, num_classes)"""
...
def create_model(in_channels: int = 3, num_classes: int = 1) -> SEResNet3D101:
    """Factory function to create SE-ResNet3D-101 model"""
    return SEResNet3D101(in_channels=in_channels, num_classes=num_classes)
