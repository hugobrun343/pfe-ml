"""
ResNet3D-101 for 3D Medical Image Classification

Architecture: ResNet-101 adapted for 3D inputs
- Input: (B, 3, 32, 256, 256) - 3-channel 3D patches
- Output: (B, num_classes) - classification logits

Layer configuration: [3, 4, 23, 3] bottleneck blocks
- Layer 1: 3 blocks, 256 channels
- Layer 2: 4 blocks, 512 channels  
- Layer 3: 23 blocks, 1024 channels (main difference from ResNet-50)
- Layer 4: 3 blocks, 2048 channels

Total parameters: ~85M (proper unique weights per block)
...
class ResNet3D101(nn.Module):
    """ResNet3D-101 for binary classification with unique weights per block"""
    
    def __init__(self, in_channels: int = 3, num_classes: int = 1, initial_channels: int = 64):
        """
        Initialize ResNet3D-101
        ...
        # ============================================================================
        # LAYER 3: (B, 512, 4, 32, 32) -> (B, 1024, 2, 16, 16) - 23 blocks
        # ============================================================================
        ...
        # Blocks 2-23 (identity): 1024 -> 1024 channels - UNIQUE WEIGHTS per block
        # Each block: conv1 (1×1×1): 1024 → 256, conv2 (3×3×3): 256 → 256, conv3 (1×1×1): 256 → 1024
        self.layer3_blocks = nn.ModuleList()
        for i in range(22):  # 22 identity blocks (total 23 blocks in layer3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # ============================ LAYER 3 =============================
        # (B, 512, 4, 32, 32) -> (B, 1024, 2, 16, 16) - 23 blocks
        ...
        # Blocks 2-23 (identity shortcut) - unique weights per block
...
def create_model(in_channels: int = 3, num_classes: int = 1) -> ResNet3D101:
    """
    Factory function to create ResNet3D-101 model
    ...
    Returns:
        ResNet3D101 model instance
    """
    return ResNet3D101(in_channels=in_channels, num_classes=num_classes)
