...
class ResultsTracker:
    ...
    def finalize(self, best_val_f1_class_1: Optional[float] = None, total_epochs: int = 0, early_stopped: bool = False) -> Path:
        """
    ...
    def _run_analysis(self, json_path: Path):
        """Run automatic analysis of training results"""
        try:
            import subprocess
            import sys
            
            script_path = Path(__file__).parent.parent / 'scripts' / 'analytics' / 'analyze_results.py'
            ...
            print(f"  python scripts/analytics/analyze_results.py {json_path}")